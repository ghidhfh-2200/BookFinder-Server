...
// 唯一索引保证同一访问者对同一字段只留一行，重复提交由数据库忽略，
...
func AddFieldReport(report *types.FieldReport) (int, error) {
	...
	err := database.GetDB().Transaction(func(tx *gorm.DB) error {
		...
		return tx.Model(&types.FieldReport{}).
			Where("library_id = ? AND field_name = ?", report.LibraryID, report.FieldName).
			Count(&count).Error
	})
	if err != nil {
		return 0, err
...
// RemoveFieldReport 撤销某访问者对某字段的报告，返回撤销后的次数与是否确有删除。
// 条件里带上 ReporterKey，故只删自己那一行、次数 -1，不会清空他人的报告。
func RemoveFieldReport(libraryID int, fieldName, reporterKey string) (int, bool, error) {
	var (
		count   int64
		removed bool
	)
	...
	err := database.GetDB().Transaction(func(tx *gorm.DB) error {
		result := tx.Where("library_id = ? AND field_name = ? AND reporter_key = ?",
			libraryID, fieldName, reporterKey).
...
// CountFieldReport 统计单个字段的独立报告次数
func CountFieldReport(libraryID int, fieldName string) (int, error) {
	var count int64
...
// CountFieldReports 统计给定图书馆各字段的报告次数，键为「图书馆 ID + 字段名」。
// 列表页一次查出所有记录的计数，避免逐行查询。
func CountFieldReports(libraryIDs []int) (map[int]map[string]int, error) {
...
// ListReportedFields 查出给定访问者在这些图书馆中已报告过的字段，
// 供前端判断该显示「报告过时」还是「撤销报告」。
func ListReportedFields(libraryIDs []int, reporterKey string) (map[int]map[string]bool, error) {
	return listFields(libraryIDs, "reporter_key = ?", reporterKey)
}

// ListSameOriginFields 查出这些图书馆中、由给定来源 IP 报告过的字段。
// 与 ListReportedFields 的差集即「同一来源报过但不是你提交的」，前端据此提前提示疑似重复。
func ListSameOriginFields(libraryIDs []int, reporterIP string) (map[int]map[string]bool, error) {
	return listFields(libraryIDs, "reporter_ip = ?", reporterIP)
}

// listFields 按给定条件查出「图书馆 ID → 字段名」的集合
func listFields(libraryIDs []int, condition string, arg string) (map[int]map[string]bool, error) {
...
// HasFieldReport 判断某访问者是否已报告过该字段
func HasFieldReport(libraryID int, fieldName, reporterKey string) (bool, error) {
	var count int64
	if err := database.GetDB().Model(&types.FieldReport{}).
		Where("library_id = ? AND field_name = ? AND reporter_key = ?",
			libraryID, fieldName, reporterKey).
		Count(&count).Error; err != nil {
		return false, err
	}
...
// CountSuspectedDuplicates 统计同一 IP 或同一指纹已就该字段报告过的次数。
// 用于启发式判断：令牌是新的但 IP 与指纹都和已有报告吻合，多半是同一人换了身份。
// 指纹可伪造，故只用来提示与拒绝计数，不作为身份依据。
func CountSuspectedDuplicates(libraryID int, fieldName, reporterIP, fingerprint string) (int64, error) {
